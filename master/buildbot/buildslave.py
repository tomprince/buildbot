...
class AbstractBuildSlave(config.ReconfigurableServiceMixin,
    ...
    def __init__(self, name, max_builds=None,
...
class PBBuildSlave(AbstractBuildSlave, pb.Avatar):
    """This is the master-side representative for a remote buildbot slave.
    There is exactly one for each slave described in the config file (the
    c['slaves'] list). When buildbots connect in (.attach), they get a
    reference to this instance. The BotMaster object is stashed as the
    ...
    def __init__(self, name, password,
                 ...
                 keepalive_interval=3600, **kwargs):
        """
        @param name: botname this machine will supply when it connects
        @param password: password this machine will supply when
                         it connects
        ...
        @type locks: dictionary
        """
        AbstractBuildSlave.__init__(self, name, **kwargs)
    ...
    def startService(self):
        ...
        return AbstractBuildSlave.startService(self)
    ...
    def reconfigService(self, new_config):
        ...
        d.addCallback(lambda _ :
            AbstractBuildSlave.reconfigService(self, new_config))

        return d

    def stopService(self):
        self.stopMissingTimer()
        return AbstractBuildSlavestopService(self)
...
class BuildSlave(PBBuildSlave):

    def sendBuilderList(self):
        d = PBBuildSlave.sendBuilderList(self)
        def _sent(slist):
...
class AbstractLatentBuildSlave(PBBuildSlave):
    ...
    def attached(self, bot):
        ...
        return PBBuildSlave.attached(self, bot)
    ...
    def _soft_disconnect(self, fast=False):
        d = PBBuildSlave.disconnect(self)
    ...
    def stopService(self):
        res = defer.maybeDeferred(PBBuildSlave.stopService, self)
        if self.slave is not None:
    ...
    def updateSlave(self):
        ...
        return PBBuildSlave.updateSlave(self)
